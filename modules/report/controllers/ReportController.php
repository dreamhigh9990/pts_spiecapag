...
class ReportController extends \yii\web\Controller
{
    ...
    public function getWelderdata($welder_name,$date,$weldType = ""){
        ...
        $Welding =  \app\models\Welding::find()->select(['welding.root_os','welding.root_ts','welding.hot_os','welding.hot_ts','welding.fill1_os',
        'welding.fill1_ts','welding.fill2_os','welding.fill2_ts','welding.fill3_os','welding.fill3_ts','welding.cap_os','welding.cap_ts','pipe.od','welding.weld_number','welding.kp'])
                    ...
                    ->where([   'OR',
                                ...
                                ['hot_os'=>$welder_name],
                                ['hot_ts'=>$welder_name],
                                ['fill1_os'=>$welder_name],
                                ['fill1_ts'=>$welder_name],
                                ['fill2_os'=>$welder_name],
                                ['fill2_ts'=>$welder_name],
                                ['fill3_os'=>$welder_name],
                                ['fill3_ts'=>$welder_name],
                                ['cap_os'=>$welder_name],
                                ['cap_ts'=>$welder_name],
                            ])
    ...
    public function actionWelderDetail(){
        ...
        $weldingDataMain =  \app\models\Welding::find()->where([
            ...
            ['welding.fill1_os'=>$welder_name],
            ['welding.fill1_ts'=>$welder_name],
            ['welding.fill2_os'=>$welder_name],
            ['welding.fill2_ts'=>$welder_name],
            ['welding.fill3_os'=>$welder_name],
            ['welding.fill3_ts'=>$welder_name],
        ...
        'defectPosition'=>array('Root OS','Root TS','Hot OS','Hot TS','Fill1 OS','Fill1 TS','Fill2 OS','Fill2 TS','Fill3 OS','Fill3 TS','Cap OS','Cap TS')]);      
    }
    ...
    public function actionWelderOverall(){
        $WeldData    = array('defect_position'=>array('Root OS','Root TS','Hot OS','Hot TS','Fill1 OS','Fill1 TS','Fill2 OS','Fill2 TS','Fill3 OS','Fill3 TS','Cap OS','Cap TS'));
        // $welderList  = ArrayHelper::map(\app\models\TaxonomyValue::find()->where(['taxonomy_id'=>7,'project_id'=>Yii::$app->user->identity->project_id])->asArray()->all(),'value','value');
    ...
    private function x($sequence){     
        ...
        $sql = "SELECT  id, weld_number, kp from welding where has_been_cut_out = 'No' AND is_deleted=0 AND is_active=1 AND welding.sequence = 0 AND project_id =".Yii::$app->user->identity->project_id ." order by kp ASC, CAST(weld_number AS UNSIGNED) ASC, id ASC limit 1";
        $data = \Yii::$app->db->createCommand($sql)->queryOne();
        if($data){
            $weld = \app\models\Welding::find()->select(['id', 'sequence', 'pipe_number', 'next_pipe', 'has_been_cut_out'])->where(['id' => $data['id']])->active()->one();
            if($weld){ 
    ...
    public function actionSequence(){
        ...
        $weldList = \app\models\Welding::updateAll( ['sequence' => 0 ],'project_id='.Yii::$app->user->identity->project_id);
        ...
        do{            
            ...
            if(!$callX){
                $weldList = \app\models\Welding::find()->select(['id', 'kp', 'sequence', 'pipe_number', 'next_pipe', 'has_been_cut_out'])
                ->where(['pipe_number' => $pipe_number, 'has_been_cut_out' => 'No', 'sequence'=>'0'])->active()->one();            
                if($weldList){
        ...
        $weldList = \app\models\Welding::updateAll( ['sequence' => 1000001 ]," has_been_cut_out='Yes' AND project_id=".Yii::$app->user->identity->project_id);
