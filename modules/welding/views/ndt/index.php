...
<?= GridView::widget([
    ...
    'columns' => [
        ...
        //     'filter' => true,
        //     'value' => function ($model) {
        //         $weldData = \app\models\Welding::find()->where(['id' => $model->main_weld_id])->active()->asArray()->one();
        //         $counts =  \app\models\Ndt::find()->where(['and', ['main_weld_id' => $model->main_weld_id], ['<', 'created_at', $model->created_at]])->active()->count();
        //         // ->andWhere(['has_been_cut_out' => 'No'])

        //         return !empty($weldData) ? ($counts ? $weldData['weld_sub_type'] . $counts : '') : '-';
        //     }
        // ],
        [
            'attribute' => 'outcome',
