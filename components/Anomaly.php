...
class Anomaly extends Component
{
	...
	public function pipe_stringing_anomaly($model,$warning=false){	
		if ($model->isNewRecord){
			...
			// $PipeExist = \app\models\Pipe::find()->where(['pipe_number'=>$model->pipe_number])->active()->one();
			$PipeExist = \app\models\Reception::find()->where(['pipe_number'=>$model->pipe_number])->active()->one();
			if(empty($PipeExist)){	
				if($warning){
					return ['status'=>true,'message'=>'Pipe number does not available in pipe reception list for this project.']	;					
				}else{			
					$model->is_anomally  = "Yes";
					$model->is_active    =  0;
					$model->why_anomally = "Pipe number does not available in pipe reception list for this project.";
				}
			}

			$CoreYieldStrength = \app\models\TaxonomyValue::find()->where(['taxonomy_id'=>12,'project_id'=>Yii::$app->user->identity->project_id])->active()->one();				
			if(!empty($CoreYieldStrength['value'])){
				$Pipe =\app\models\Reception::find()->where(['pipe_number'=>$model->pipe_number])->active()->one();
