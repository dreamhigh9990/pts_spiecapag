...
class Export extends Component
{
            ...
            foreach ($data as $kEle => $ele) {
                ...
                foreach ($ele as $key => $val) {
                    ...
                    if ($type == 'pipe') {
                        ...
                        // if($key == 'bending_checkpoints') continue;
                        // if($key == 'pull_through_accepted') continue;
                        if ($key == 'pipe_id') continue;
                        if ($key == 'kp') continue;
                        if ($key == 'project_id') continue;

                        $v[$key] = $val;
                        if ($key == 'bending_checkpoints') {
                            $v[$key] = str_replace("[", "", $v[$key]);
                            $v[$key] = str_replace("]", "", $v[$key]);
                            $v[$key] = str_replace("\"", "", $v[$key]);
                        }
                        if ($key == 'pipe_number') {
                            $receptionAdditionalData = $this->additioFieldsForBending($val);
                            if (!empty($receptionAdditionalData)) {
                                foreach ($receptionAdditionalData as $addiKey => $addiVal) {
                                    $v[$addiKey] = $addiVal;
                                }
    ...
    public function additioFieldsForBending($pipeNumber)
    {
        $pipeDetails = \app\models\Pipe::find()->select(['defects'])->where(['pipe_number' => $pipeNumber])->active()->asArray()->one();
