...
?>
	...
	public function pipeFiled($model, $form)
	{
		if (get_class($model) == "app\models\Stringing") {
			$class = "auto-pipe-from-reception form-control"; //actionPipeAutoListForStringing
				...
				<div class="form-group clearfix ddd">
					<div class="col-md-6 clearfix">
						<?= Html::dropDownList(
							'Welding[defects][]',
							$def['defects'],
							Yii::$app->general->TaxonomyDrop(9),
					...
					<div class="form-group clearfix">
						...
						<div class="col-md-12 clearfix">
							<?= Html::dropDownList(
								'Pipe[defects][]',
								$def,
								Yii::$app->general->TaxonomyDrop(8),
	...
	{	?>
		...
		<?php
		if (
			get_class($model) == "app\models\Reception"
	...
	public function getModelData($model, $EditId)
	{
		...
		if (!empty($model)) {
			...
			return array('status' => false, 'message' => 'This item is not exist in this project');
			die;
		}
	}
	...
	public function downloadCsvforweldbook($data)
	{
		...
		if ($data) {
			foreach ($data as $ele) {
				...
				$pipeBendAngle = \app\models\Bending::find()->select(['angle'])->where(['pipe_number' => $ele['pipe_number']])->active()->asArray()->one();
				$nextpipeBendAngle = \app\models\Bending::find()->select(['angle'])->where(['pipe_number' => $ele['next_pipe']])->active()->asArray()->one();
				$nextpipeYieldStrength = \app\models\Pipe::find()->select(['yeild_strength'])->where(['pipe_number' => $ele['next_pipe']])->active()->asArray()->one();
				...
				foreach ($ele as $key => $val) {
					...
					if ($key == 'pipe_number') {
						...
						$v['Bend Angle'] =  !empty($pipeBendAngle['angle']) ? $pipeBendAngle['angle'] : "-";
					}

					if ($key == 'next_pipe') {
						$v[$key] =  $val;
						$v['Next Bend Angle'] =  !empty($nextpipeBendAngle['angle']) ? $nextpipeBendAngle['angle'] : "-";
						$v['Yield Strength'] =  !empty($nextpipeYieldStrength['yeild_strength']) ? $nextpipeYieldStrength['yeild_strength'] : "-";
					}
	...
	public function globalDownload($query, $app = false)
	{
		...
		if ($app) {
			return Yii::$app->request->hostInfo . Yii::getAlias('@web') . '/excel/' . $filename;
			die;
		}
	...
	public function getWeldAll($filtertype, $daterange)
	{
		...
		if ($filtertype == "weekly") {
			...
			$endDate   = date('Y-m-d');
			$startDate = date('Y-m-d', strtotime("-10 year", strtotime($endDate)));
		}
		foreach ($joins as $join) {

			$TotalWeldCount = 0;
			$TotalWeldLength = 0;
			$TotalWeldRepairCount = 0;
			$Welding =  \app\models\Welding::find()->select([
				'welding.date', 'welding.root_os', 'welding.root_ts', 'welding.hot_os', 'welding.hot_ts', 'welding.fill1_os',
				'welding.fill1_ts', 'welding.fill2_os', 'welding.fill2_ts', 'welding.fill3_os', 'welding.fill3_ts', 'welding.cap_os', 'welding.cap_ts', 'welding.weld_number', 'welding.kp'
			])
			...
			if (!empty($Welding[0])) {
				...
				foreach ($Welding as $Weld) {
					...
					$pos = array('root_os', 'root_ts', 'hot_os', 'hot_ts', 'fill1_os', 'fill1_ts', 'fill2_os', 'fill2_ts', 'fill3_os', 'fill3_ts', 'cap_os', 'cap_ts');
	...
	public function getWeldRepairAll()
	{
		...
		$MianLineWeldRepair = \app\models\Weldingrepair::find()
			->leftJoin(
				'welding',
				'welding_repair.weld_number=welding.weld_number 
			AND welding_repair.kp=welding.kp 
