...
class ParameterSearch extends Parameter
{
    ...
    public function rules()
    {
        return [
            ...
            [['rot', 'k_factor', 'wire_speed'], 'number'],
            [['kp'], 'string'],
    ...
    public function search($params)
    {
        if(Yii::$app->controller->module->id == "admin" && Yii::$app->controller->id == "anomaly"){
            $query = Parameter::find()->anomally()->orderBy($this->sortOption($params,'id DESC'));
        }else{
             $query = Parameter::find()->active()->orderBy($this->sortOption($params,'id DESC'));
        }
