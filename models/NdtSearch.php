...
class NdtSearch extends Ndt
{
    ...
    public function rules()
    {
        return [
            ...
            [['date', 'report_number', 'weld_number', 'outcome', 'comment', 'is_anomally', 'why_anomally', 'signed_off', 'weldType', 'weld_sub_type'], 'safe'],
            [['kp'], 'string'],
    ...
    public function search($params)
    {
        if(Yii::$app->controller->module->id == "admin" && Yii::$app->controller->id == "anomaly"){
            $query = Ndt::find()->anomally()->orderBy($this->sortOption($params,'id DESC'));
        }else{
            $query = Ndt::find()->active()->orderBy($this->sortOption($params,'id DESC'));
        }
        ...
        if(!empty($params['NdtSearch']) && (!empty($params['NdtSearch']['weldSubType']) || !empty($params['NdtSearch']['weldType']))){
            ...
            $query = Ndt::find()->select(['welding_ndt.*', 'welding.weld_type'])->leftJoin('welding', 'welding_ndt.weld_number = welding.weld_number AND welding_ndt.project_id = welding.project_id AND welding.is_active = 1 AND welding.is_deleted = 0 AND welding_ndt.kp = welding.kp AND welding.project_id = '.Yii::$app->user->identity->project_id)->active()->orderBy('id DESC');
        }
        ...
        if(isset($this->weld_sub_type) && $this->weld_sub_type!=""){
            $query->andFilterWhere(['=', 'welding_ndt.weld_sub_type', $this->weld_sub_type]);
        }
