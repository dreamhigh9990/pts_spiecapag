...
class ProductionSearch extends Production
{
    ...
    public function rules()
    {
        return [
            ...
            [['dew_point', 'temperature', 'surface_profile', 'dft', 'dft_2', 'dft_3', 'dft_4', 'dft_5', 'dft_6'], 'number'],
            [['kp'], 'string'],
    ...
    public function search($params)
    {
        if(Yii::$app->controller->module->id == "admin" && Yii::$app->controller->id == "anomaly"){
            ...
            $query = Production::find()->anomally()->orderBy($this->sortOption($params,'id DESC'));
        }else{
            // $query = Production::find()->active()->orderBy('kp ASC');
            $query = Production::find()->active()->orderBy($this->sortOption($params,'id DESC'));
        }
