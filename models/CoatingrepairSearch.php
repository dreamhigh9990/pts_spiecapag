...
class CoatingrepairSearch extends Coatingrepair
{
    ...
    public function rules()
    {
        return [
            ...
            [['ambient_temperature', 'substrate_temprature', 'humidity', 'dew_point', 'coating_product', 'temperature', 'time'], 'number'],
            [['kp'], 'string'],
    ...
    public function search($params)
    {
        if(Yii::$app->controller->module->id == "admin" && Yii::$app->controller->id == "anomaly"){
            $query = Coatingrepair::find()->anomally()->orderBy($this->sortOption($params,'id DESC'));
        }else{
            $query = Coatingrepair::find()->active()->orderBy($this->sortOption($params,'id DESC'));
        }
