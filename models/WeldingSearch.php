...
class WeldingSearch extends Welding
{
    ...
    public function rules()
    {
        return [
            ...
            [['date', 'report_number', 'line_type', 'pipe_number', 'next_pipe', 'geo_location', 'weld_type', 'weld_crossing', 'weld_number', 'weld_sub_type', 'WPS', 'electrodes', 'root_os', 'root_ts', 'hot_os', 'hot_ts', 'fill1_os', 'fill1_ts', 'fill2_os', 'fill2_ts',  'fill3_os', 'fill3_ts', 'cap_os', 'cap_ts', 'visual_acceptance', 'comment', 'signed_off', 'is_anomally'], 'safe'],
            [['kp', 'has_been_cut_out'], 'string'],
            // [['end_kp', 'end_weld'], 'number'],
            // ['kp', 'match', 'pattern' => '/^[a-z,A-Z]-(\d)+$/', 'message' => 'You must input the value to keep format. ex:A-0 '],
        ];
    }
    ...
    private function sortOption($params, $defstr)
    {
        if (!isset($params['sort'])) {
            return $defstr;
        }
        if (substr($params['sort'], 0, 1) == '-') {
            return substr($params['sort'], 1) . " DESC";
        } else {
    ...
    public function search($params)
    {
        ...
        if (Yii::$app->controller->module->id == "admin" && Yii::$app->controller->id == "anomaly") {
            ...
            $query = Welding::find()->active()->orderBy($this->sortOption($params, 'id DESC'));
        }
        // add conditions that should always apply here
        if (Yii::$app->controller->module->id  == "report") {
            $query = Welding::find()->select('id, date, report_number, geo_location, kp, weld_number, line_type, pipe_number, next_pipe, sequence, visual_acceptance, has_been_cut_out, weld_type, weld_crossing, root_os, root_ts, hot_os, hot_ts, fill1_os, fill1_ts, fill2_os, fill2_ts,  fill3_os, fill3_ts,cap_os, cap_ts, WPS, weld_sub_type, electrodes, comment, created_by, signed_off, qa_manager, created_at, updated_at')->active()->orderBy($this->sortOption($params, 'sequence ASC'));
        }
        ...
        $query->andFilterWhere(['like', 'report_number', $this->report_number])
            ...
            ->andFilterWhere(['like', 'fill1_os', $this->fill1_os])
            ->andFilterWhere(['like', 'fill1_ts', $this->fill1_ts])
            ->andFilterWhere(['like', 'fill2_os', $this->fill2_os])
            ->andFilterWhere(['like', 'fill2_ts', $this->fill2_ts])
            ->andFilterWhere(['like', 'fill3_os', $this->fill3_os])
            ->andFilterWhere(['like', 'fill3_ts', $this->fill3_ts])
